...
The system SHALL display all products in a sortable, paginated DataTable at `/admin/products`. Each row SHALL show: product name, category name, price (formatted as currency), availability status, and action buttons (Edit, Delete).

#### Scenario: Product list renders server-side
- **WHEN** an admin navigates to `/admin/products`
- **THEN** the page renders with a DataTable pre-populated with products fetched on the server

#### Scenario: Admin sorts by column
- **WHEN** the admin clicks a sortable column header
- **THEN** the DataTable re-orders rows by that column (ascending, then descending on second click)

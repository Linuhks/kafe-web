...
- **THEN** "Bem-vindo de volta" heading and "Entre para continuar sua jornada no café." text are in the document

#### Scenario: Form fields render
- **WHEN** the login page is rendered
- **THEN** email input, password input, and "Entrar" button are present

#### Scenario: Navigation links render
- **WHEN** the login page is rendered
- **THEN** "Esqueceu a senha?" and "Criar uma conta" links are present
...
- **WHEN** the login page is rendered
- **THEN** "ou junte-se ao clube" text is present

#### Scenario: Footer renders
- **WHEN** the login page is rendered
- **THEN** "© 2024 Kafe Roastery. Todos os direitos reservados." text is present

#### Scenario: Hero image has accessible alt text
- **WHEN** the login page is rendered
- **THEN** an image with alt "O Ritual do Preparo" is present
...
- **WHEN** the login page is rendered
- **THEN** the password input has type "password" and the toggle button has aria-label "Mostrar senha"

#### Scenario: Toggle reveals password
- **WHEN** user clicks the "Mostrar senha" toggle button
- **THEN** the password input type changes to "text" and aria-label becomes "Ocultar senha"
...
- **THEN** "E-mail inválido" error message appears in the document

#### Scenario: Short password shows error
- **WHEN** user submits the form with a password shorter than 8 characters
- **THEN** "A senha deve ter pelo menos 8 caracteres" error message appears
...
- **THEN** addToast is called with "E-mail ou senha inválidos" and type "error"

#### Scenario: Network error shows generic toast
- **WHEN** the mutateAsync call throws an exception
- **THEN** addToast is called with "Falha no login. Tente novamente." and type "error"
...
- **THEN** the submit button text is "Entrando…" and the button is disabled
